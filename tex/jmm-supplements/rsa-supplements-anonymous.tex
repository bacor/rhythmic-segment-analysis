...
Formally, this is taken care of by the function
\begin{align}
...
Knowing how to move between segment space and pattern-duration space, we can pull the taxicab distance from the segment space to define the \emph{pattern-duration distance $d_{pd}$} on the pattern-duration space as
\begin{align}
...
\begin{definition}
  The \emph{anisochrony} of a pattern $\vp$ is the normalized distance from isochrony, represented by the centroid $\vc$, is defined as follows.
  \begin{align}
    \alpha(\vp)
